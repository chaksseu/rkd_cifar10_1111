...
class SRProjectorMLP(nn.Module):
    """
    Simple MLP projector for S_f and R_f only (input_dim == output_dim).
    Default: residual MLP so it starts close to identity.
    """
    def __init__(self, dim: int, hidden_mult: float = 2.0, dropout: float = 0.0):
        super().__init__()
        hidden = max(1, int(dim * hidden_mult))
        ...
        # start near identity (residual) by zero-init last layer
...
def infer_feature_dim(embedder: nn.Module, image_size: int, device: torch.device) -> int:
    with torch.no_grad():
...
def compute_losses(
    ...
    args,
    sr_projector: Optional[nn.Module] = None,  # NEW
):
    eps = 1e-12
    ...
    if sr_projector is not None:
        S_f = sr_projector(S_f)
        R_f = sr_projector(R_f)
    ...
    if args.w_fid != 0.0:
        fid_s = frechet_distance_diag(S_f_RAW.float(), R_f_RAW.float(), eps=args.fid_eps)
...
def train(args):
    ...
    # ---- SR projector (trainable) ----
    sr_projector = None
    sr_proj_dim = None
    if not args.no_sr_projector:
        sr_proj_dim = infer_feature_dim(embedder, args.image_size, device)
        sr_projector = SRProjectorMLP(
            dim=sr_proj_dim,
            hidden_mult=args.sr_proj_hidden_mult,
            dropout=args.sr_proj_dropout,
        ).to(device)
        sr_projector.train()
        print(f"[Info] SRProjectorMLP enabled: dim={sr_proj_dim}, hidden_mult={args.sr_proj_hidden_mult}", flush=True)
    else:
        print("[Info] SRProjectorMLP disabled.", flush=True)
    ...
    if args.resume_checkpoint and os.path.exists(args.resume_checkpoint):
        ...
        # --- load SR projector if exists ---
        if sr_projector is not None:
            proj_path = Path(args.resume_checkpoint) / "sr_projector.pt"
            if proj_path.exists():
                ckpt = torch.load(proj_path.as_posix(), map_location="cpu")
                state = ckpt["state_dict"] if (isinstance(ckpt, dict) and "state_dict" in ckpt) else ckpt
                try:
                    sr_projector.load_state_dict(state, strict=True)
                    print(f"[Info] Loaded SRProjectorMLP from {proj_path}", flush=True)
                except Exception as e:
                    print(f"[Warn] Failed to load SRProjectorMLP ({e}). Re-init projector.", flush=True)
            else:
                print(f"[Warn] No SRProjectorMLP checkpoint at {proj_path}. Re-init projector.", flush=True)
    ...
    if sr_projector is not None:
        trainable_params += [p for p in sr_projector.parameters() if p.requires_grad]
        print(f"[Info] Student params: {count_parameters(sr_projector):,}", flush=True)
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for it, x0_real in enumerate(loader, start=1):
            ...
            if args.save_interval > 0 and (global_step % args.save_interval == 0):
                ...
                if sr_projector is not None:
                    torch.save(
                        {
                            "state_dict": sr_projector.state_dict(),
                            "dim": sr_proj_dim,
                            "hidden_mult": float(args.sr_proj_hidden_mult),
                            "dropout": float(args.sr_proj_dropout),
                        },
                        (save_dir / "sr_projector.pt").as_posix(),
                    )
        ...
        if sr_projector is not None:
            torch.save(
                ...
                (last_dir / "sr_projector.pt").as_posix(),
            )
...
def build_argparser():
    ...
    p.add_argument("--output_dir", type=str, default=f"out_{DATE}_rkd/rkd_{RKD_METRIC}_lora_feature_cifar10_rgb_to_gray_single_batch{BATCH_SIZE}_N{CLASSN}_LR{LR}-EASY_FD-rkdW{RKD_W}-invW{INV_W}-invinvW{INVINV_W}-fdW{FD_W}-teacher-init-eps-NN")
    p.add_argument("--run_name", type=str, default=f"student-lora-{RKD_METRIC}-x0-rgb-to-gray-batch{BATCH_SIZE}-N{CLASSN}-LR{LR}-FD-rkdW{RKD_W}-invW{INV_W}-invinvW{INVINV_W}-fdW{FD_W}-teacher-init-eps-EASY-FD-NN")
    ...
    p.add_argument("--no_sr_projector", action="store_true",
                   help="Disable learnable projector on (S_f, R_f) for RKD/INV/INVINV.")
    p.add_argument("--sr_proj_hidden_mult", type=float, default=2.0)
    p.add_argument("--sr_proj_dropout", type=float, default=0.0)
