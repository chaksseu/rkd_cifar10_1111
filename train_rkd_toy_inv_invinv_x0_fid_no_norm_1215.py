...
Student (x0 predictor) distillation with pixel-space losses ONLY (SINGLE GPU, NO accelerate, NO grad accumulation):
  ...
  - FID loss (Gaussian FID) computed on Inception features (pytorch-fid InceptionV3), NOT on raw pixels
...
- For train-time Inception-FID loss and eval FID you need: pip install pytorch-fid
"""
...
def mean_from_vectors(vectors: List[torch.Tensor], device: torch.device, eps: float = 1e-12) -> torch.Tensor:
    """
    Single-GPU mean over a list of 1D tensors, used for mean-normalization.
    ...
    for v in vectors:
        vv = v  # NOTE: as-is (no detach), matching your current style
...
# ------------------------- Inception Feature Extractor (TRAIN FID loss) -------------------------

class InceptionV3Feature(nn.Module):
    """
    Extract InceptionV3 pooled features (e.g., 2048-d) compatible with pytorch-fid.

    Input:  x in [-1, 1], shape (N,3,H,W)
    Output: feats shape (N, dims)
    """
    def __init__(self, dims: int = 2048):
        ...
        try:
            ...
            raise RuntimeError(
                "Train-time Inception FID loss requires pytorch-fid.\n"
                "Install: pip install pytorch-fid"
            ) from e

        if dims not in InceptionV3.BLOCK_INDEX_BY_DIM:
            raise ValueError(
                f"Unsupported dims={dims}. Supported: {sorted(InceptionV3.BLOCK_INDEX_BY_DIM.keys())}"
            )

        block_idx = InceptionV3.BLOCK_INDEX_BY_DIM[dims]
        self.inception = InceptionV3(
            [block_idx],
            resize_input=True,      # resize to 299
            normalize_input=True,   # expects input in [0,1], internally maps to [-1,1]
            requires_grad=False,
        )
        self.dims = dims
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feat = self.inception(x01)[0]           # (N, dims, 1, 1)
        feat = feat.squeeze(-1).squeeze(-1)     # (N, dims)
        return feat


# ------------------------- Gaussian FID (used on Inception features; differentiable w.r.t. features) -------------------------
...
def invert_x0_to_zT_deterministic_x0pred(student, ddim_S, x0, steps, device) -> torch.Tensor:
    """
    Uses DDIMInverseScheduler to invert from x0 to a latent/noise-like representation.
    (Kept in your current style.)
    """
    ...
    for t in inv.timesteps:
        ...
        x0_pred = student(latent_in, t_b).sample
        xt = inv.step(x0_pred, t, xt).prev_sample
...
def compute_losses(
    ...
    args,
    fid_extractor: Optional[nn.Module] = None,
):
    eps = 1e-12
    ...
    if args.w_inv != 0.0:
        ...
        inv_t = cdist_vec(flatten_img(T_last).float(), flatten_img(x0_inv_T).float(), eps=eps)

    invinv_s = invinv_t = None
    if args.w_invinv != 0.0:
        invinv_s = pdist_vec(flatten_img(x0_real).float(), eps=eps)
        invinv_t = pdist_vec(flatten_img(x0_inv_T).float(), eps=eps)
    ...
    # ---- TRAIN loss FID (Gaussian FID on Inception features) ----
    loss_fid = torch.tensor(0.0, device=device)
    fid_s = fid_t = torch.tensor(0.0, device=device)

    if args.w_fid != 0.0:
        if fid_extractor is None:
            raise RuntimeError("fid_extractor is None but w_fid != 0. Initialize InceptionV3Feature in train().")

        # Ensure no AMP for Inception + linalg
        amp_off = (
        ...
        with amp_off:
            # student term: grad flows through S_last
            S_f = fid_extractor(S_last)  # (B, dims)

            # real features: no grad
            with torch.no_grad():
                R_f = fid_extractor(x0_real)  # (B, dims)

            # teacher-last features: no grad (T_last already detached)
            with torch.no_grad():
                T_f = fid_extractor(T_last)   # (B, dims)

            # inverted target: keep grad if x0_inv_T carries grad in your pipeline
            I_f = fid_extractor(x0_inv_T)     # (B, dims)
...
def train(args):
    ...
    # ---- Inception feature extractor for TRAIN FID loss ----
    fid_extractor = None
    if args.w_fid != 0.0:
        fid_extractor = InceptionV3Feature(dims=args.fid_dims).to(device)
        fid_extractor.eval()
        for p in fid_extractor.parameters():
            p.requires_grad = False
        print(f"[Info] Train FID loss uses Inception features dims={args.fid_dims}", flush=True)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for it, x0_real in enumerate(loader, start=1):
            ...
            # inversion + teacher on inverted latent
            ...
            x0_inv_T = preds_T_inv[-1]  # (your current style; no detach)
...
def build_argparser():
    ...
    # FID eps (Gaussian FID on features)
    ...
    p.add_argument("--fid_dims", type=int, default=2048, help="Inception feature dims (default 2048). Also used for TRAIN FID loss.")
