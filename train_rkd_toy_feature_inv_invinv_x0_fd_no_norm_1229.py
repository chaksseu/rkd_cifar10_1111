...
"""
Student (x0 predictor) distillation with Feature-based losses (Pixel, DINOv3, CLIP):
  - RKD (pdist) in Feature space
  - INV (cdist) in Feature space
  - INVINV (pdist) in Feature space
  - FID loss (Gaussian) in Feature space
  - SAME loss (trajectory shrink regularizer)

Dependencies:
  - pip install diffusers transformers torch torchvision pytorch-fid accelerate
  - pip install "transformers>=4.56.0"  # Required for DINOv3
"""
...
class FeatureEmbedder(nn.Module):
    ...
    Modes:
      ...
      - 'dinov3': DINOv3 embeddings (via AutoModel). Resizes to 224x224.
    ...
    def __init__(self, mode: str, device: torch.device, 
        ...
        if mode == 'pixel':
            ...
            try:
                ...
                raise RuntimeError(f"Failed to load DINOv3 model '{dino_model_name}'.\nError: {e}")

            for p in self.net.parameters():
                p.requires_grad = False
            self.net.eval()
            
            # DINOv3 uses ImageNet mean/std
            ...
            try:
                # We only need the vision model part of CLIP
                self.net = CLIPModel.from_pretrained(clip_model_name).vision_model.to(device)
            except Exception as e:
                raise RuntimeError(f"Failed to load CLIP model '{clip_model_name}'.\nError: {e}")
            ...
            raise ValueError(f"Unknown metric mode: {mode}. Supported: 'pixel', 'dinov3', 'clip'.")
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.mode == 'dinov3':
            ...
            if hasattr(outputs, "pooler_output") and outputs.pooler_output is not None:
                print("dinov3 outputs.pooler_output shape", outputs.pooler_output.shape)
                return outputs.pooler_output
            else:
                print("dinov3 outputs shape", outputs.shape)
                print("dinov3 outputs.last_hidden_state[:, 0, :] shape", outputs.last_hidden_state[:, 0, :].shape)
        ...
        if self.mode == 'clip':
            ...
            outputs = self.net(pixel_values=x_norm)
            # CLIP vision model returns pooler_output (embedding)
            return outputs.pooler_output
...
def compute_losses(
    ...
    if args.w_invinv != 0.0:
        f_real = get_feats(x0_real) if (args.w_inv == 0.0) else f_real 
...
def train(args):
    ...
    # ---- Student (FULL TRAIN, NO LORA) ----
    print(f"[Info] Initializing Student (Full Fine-Tuning)...", flush=True)
    if args.student_dir != "":
...
BATCH_SIZE = 8
CLASSN = 10
RKD_METRIC="dinov3" # pixel, dinov3, clip
CUDA_NUM = 7
LR=1e-5
DATE="1229"
...
def build_argparser():
    p = argparse.ArgumentParser("Student x0 distillation with Feature-based losses (Pixel/DINOv3/CLIP)")
    ...
    p.add_argument("--output_dir", type=str, default=f"out_{DATE}_rkd/rkd_{RKD_METRIC}_feature_cifar10_rgb_to_gray_single_batch{BATCH_SIZE}_N{CLASSN}_LR{LR}-FD-rkdW{RKD_W}-invW{INV_W}-invinvW{INVINV_W}-fdW{FD_W}-sameW{SAME_W}-teacher-init-eps_N10")

    # Metric Selection for RKD/INV
    p.add_argument("--rkd_metric", type=str, default=RKD_METRIC, choices=["pixel", "dinov3", "clip"], 
                   help="Metric space for RKD/INV losses: 'pixel', 'dinov3', 'clip'.")
    
    # DINOv3 option
    #  #facebook/dinov3-vitl16-pretrain-lvd1689m" # facebook/dinov3-vitb16-pretrain-lvd1689m
    ...
    p.add_argument("--clip_model_name", type=str, default="openai/clip-vit-base-patch32", 
                   help="HuggingFace model name for CLIP.")
    ...
    p.add_argument("--run_name", type=str, default=f"student-{RKD_METRIC}-rgb-to-gray-batch{BATCH_SIZE}-N{CLASSN}-LR{LR}-FD-rkdW{RKD_W}-invW{INV_W}-invinvW{INVINV_W}-fdW{FD_W}-sameW{SAME_W}-teacher-init-eps_N10")
