...
def x0_gray3_from_x0(x):
    """
    x: Tensor (B,C,H,W) with C in {1,3} OR List/Tuple of such tensors
    Returns: same type, but always (B,3,H,W) where all 3 channels are identical.
    Assumes input is in [-1,1] (x0 space). Output also stays in [-1,1].
    """
    if isinstance(x, (list, tuple)):
        return [x0_gray3_from_x0(xx) for xx in x]

    if not torch.is_tensor(x):
        raise TypeError(f"x0_gray3_from_x0 expects Tensor or list/tuple of Tensor, got {type(x)}")

    if x.ndim != 4:
        raise ValueError(f"x0_gray3_from_x0 expects 4D tensor (B,C,H,W), got {x.shape}")

    if x.shape[1] == 3:
        g = x.mean(dim=1, keepdim=True)          # (B,1,H,W)
    elif x.shape[1] == 1:
        g = x
    else:
        raise ValueError(f"Unsupported channel count: C={x.shape[1]} (expected 1 or 3)")

    return g.repeat(1, 3, 1, 1)                  # (B,3,H,W), all channels identical
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for it, x0_real_depth in enumerate(train_loader, start=1):
            ...
            # x0_inv_T = x0_gray3_from_x0(x0_inv_T)
...
def build_argparser():
    ...
    p.add_argument(
        "--output_dir",
        type=str,
        default=f"/workspace/rkd_cifar10_1111/{DATE}_out_rgb2depth_rkd_affine_eval/{RKD_METRIC}_RKD{RKD_W}_INV{INV_W}_INVINV{INVINV_W}_FD_{FD_W}_SAME_{SAME_W}-ch1-before-rkd",
    )
    ...
    p.add_argument("--run_name", type=str, default=f"student-lora-depth-{RKD_METRIC}-RKD{RKD_W}_INV{INV_W}_INVINV{INVINV_W}_FD_{FD_W}_SAME_{SAME_W}-ch1-before-rkd")
