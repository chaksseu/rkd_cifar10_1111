...
def unet_predx0_seq_latent_with_grad(
    unet: UNet2DConditionModel,
    scheduler: DDIMScheduler,
    z_lat: torch.Tensor,
    steps: int,
...
def compute_losses(
    preds_T_lat: List[torch.Tensor],
    preds_S_lat: List[torch.Tensor],
    ...
    with torch.no_grad():
        T_last_img = decode_latents_to_images(vae, preds_T_lat[-1], vae_scaling)
    S_last_img = decode_latents_to_images(vae, preds_S_lat[-1], vae_scaling)
    ...
    if args.w_rkd != 0.0:
        ...
        if args.rkd_teacher_ref == "last":
            with torch.no_grad():
                T_ref = pdist_vec(get_feats(T_last_img), eps=eps)
            for k in range(0, len(preds_S_lat), stride):
                
                k = random.randrange(len(preds_S_lat))

                S_img_k = decode_latents_to_images(vae, preds_S_lat[k], vae_scaling)
                rkd_s_list.append(pdist_vec(get_feats(S_img_k), eps=eps))
                rkd_t_list.append(T_ref)
            ...
            for k in range(0, len(preds_S_lat), stride):
                S_img_k = decode_latents_to_images(vae, preds_S_lat[k], vae_scaling)
                with torch.no_grad():
                    T_img_k = decode_latents_to_images(vae, preds_T_lat[k], vae_scaling)
                rkd_s_list.append(pdist_vec(get_feats(S_img_k), eps=eps))
                rkd_t_list.append(pdist_vec(get_feats(T_img_k), eps=eps))

    # INV / INVINV
    inv_s = inv_t = None
    if args.w_inv != 0.0:
        inv_s = cdist_vec(get_feats(S_last_img), get_feats(x0_real_img), eps=eps)
        inv_t = cdist_vec(get_feats(T_last_img), get_feats(x0_inv_T_img), eps=eps)

    invinv_s = invinv_t = None
    if args.w_invinv != 0.0:
        invinv_s = pdist_vec(get_feats(x0_real_img), eps=eps)
        invinv_t = pdist_vec(get_feats(x0_inv_T_img), eps=eps)

    # mean normalization
    student_parts, teacher_parts = [], []
    if args.w_rkd != 0.0 and len(rkd_s_list) > 0:
        student_parts.append(torch.cat(rkd_s_list, dim=0))
        teacher_parts.append(torch.cat(rkd_t_list, dim=0))
    ...
    if args.w_rkd != 0.0 and len(rkd_s_list) > 0:
        loss_rkd = sum(F.mse_loss(ds, dt, reduction="mean") for ds, dt in zip(rkd_s_list, rkd_t_list)) / max(1, len(rkd_s_list))
    ...
    if args.w_fid != 0.0:
        ...
        R_f = get_feats(x0_real_img).float()
        T_f = get_feats(T_last_img).float()
        I_f = get_feats(x0_inv_T_img).float()
    ...
    if args.w_same != 0.0:
        xs = torch.stack(preds_S_lat, dim=0)  # [K,B,4,h,w]
    ...
    total = args.w_rkd * loss_rkd + args.w_inv * loss_inv + args.w_invinv * loss_invinv + args.w_fid * loss_fid + args.w_same * loss_same
...
def train(args):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for it, (x0_real_img, prompts) in enumerate(loader, start=1):
            ...
            # student preds (grad)
            with autocast_ctx:
                preds_S_lat = unet_predx0_seq_latent_with_grad(
                    student, ddim, z, steps=ddim_steps_train, eta=args.ddim_eta, device=device, cond=cond
                )

            # inversion target: use TEACHER for inversion (stable, fixed target)
            # with torch.no_grad():
            x0_real_lat = encode_images_to_latents(vae, x0_real_img, scaling_factor=vae_scaling)
            zT_real = invert_x0latent_to_noise(
                teacher, ddim, x0_real_lat, steps=ddim_steps_train, device=device, cond=cond
            )
            preds_T_inv_lat = unet_predx0_seq_latent_with_grad(
                teacher, ddim, zT_real, steps=ddim_steps_train, eta=args.ddim_eta, device=device, cond=cond
            )
            x0_inv_T_img = decode_latents_to_images(vae, preds_T_inv_lat[-1], vae_scaling)
...
def build_argparser():
    ...
    p.add_argument("--output_dir", type=str, default="0114_rkd_sd_cifar10_graytest")
    ...
    p.add_argument("--run_name", type=str, default="student-lora-sd15")
    ...
    p.add_argument("--real_batch", type=int, default=4)
    ...
    # ddim
    p.add_argument("--ddim_steps_min", type=int, default=18)
    p.add_argument("--ddim_steps_max", type=int, default=22)
    ...
    # misc loss configs
    p.add_argument("--rkd_stride", type=int, default=18)  # SD decode cost 줄이기
